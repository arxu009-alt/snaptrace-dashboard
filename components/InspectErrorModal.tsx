...
3. 🛠️ Proposed Code Patch: Corrected code snippet to fix the issue.`;
      ...
      // 1. Google Gemini (Supports all AQ. and AIza keys via Dynamic Model Discovery)
      if (savedProvider === 'gemini' || key.startsWith('AQ') || key.startsWith('AIza') || !key.startsWith('sk-')) {
        let selectedModel = 'models/gemini-1.5-flash';

        try {
          // Dynamic Discovery: Ask Google which models are enabled on this exact key
          ...
          if (modelListData.models && Array.isArray(modelListData.models)) {
            const supported = modelListData.models.find((m: any) =>
              m.supportedGenerationMethods?.includes('generateContent') &&
              (m.name.includes('flash') || m.name.includes('gemini'))
            );
            if (supported) {
              selectedModel = supported.name; // e.g. "models/gemini-2.0-flash" or "models/gemini-1.5-flash-latest"
            }
          }
        } catch (e) {
        ...
        // Clean model name if already formatted
        const endpointModel = selectedModel.startsWith('models/') ? selectedModel : `models/${selectedModel}`;

        const res = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/${endpointModel}:generateContent?key=${key}`,
          {
            method: 'POST',
            headers: {
        ...
        if (!res.ok) {
          throw new Error(data.error?.message || 'Google Gemini could not process the key.');
        }

        analysisText =
          data.candidates?.[0]?.content?.parts?.[0]?.text ||
          'No diagnosis generated by model.';
      } 
