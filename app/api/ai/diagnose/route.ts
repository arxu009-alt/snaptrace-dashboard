...
3. 🛠️ Proposed Code Patch: Corrected code snippet to fix the issue.`;
    ...
    // 1. Google Gemini Provider (Lightning Fast: Races top 2 flash models simultaneously)
    if (provider === 'gemini' || key.startsWith('AQ') || key.startsWith('AIza') || !key.startsWith('sk-')) {
      const fastModels = ['gemini-2.5-flash-lite', 'gemini-2.0-flash'];
      ...
      const promises = fastModels.map(async (model) => {
        ...
        throw new Error(data.error?.message || `Failed on ${model}`);
      });

      try {
        // Race both models simultaneously; returns the fastest successful answer in 1-2 seconds!
        const fastestResult = await Promise.any(promises);
        return NextResponse.json({ success: true, analysis: fastestResult });
      } catch (aggregateErr: any) {
        return NextResponse.json(
          { error: 'Google Gemini request failed. Please verify your API key in Settings.' },
          { status: 400 }
        );
      }
    }
